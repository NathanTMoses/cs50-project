...
def index():
    ...
    if name == "Owner":
        mod = True
        return render_template("index.html", name=name, text=text, lists=lists, mail=mail, new=new, mod=mod, users=users, user=user)
    return render_template("index.html", name=name, text=text, lists=lists, mail=mail, new=new, users=users, user=user)
...
def register():
    ...
    if request.method == "POST":
        ...
        result = db.execute("INSERT INTO users (id, username, hash)  VALUES(:id, :username, :hash)", id=new_id, username=request.form.get("username"), hash=request.form.get("password"))
        if not result:
...
def settings():
    ...
    return render_template("settings.html", name=name, word=word)
...
def textedit():
    if not request.form.get("new"):
        ...
        text = db.execute("SELECT * FROM texts WHERE title = :title", title=title)[0]['text_message']
